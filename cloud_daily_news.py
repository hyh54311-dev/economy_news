...
def send_telegram_message(text):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        log_message("⚠️ 텔레그램 토큰 또는 채팅 ID가 설정되지 않았습니다.")
        return
    
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    
    # 메시지가 너무 길 경우 잘라서 전송 (텔레그램 제한: 4096자)
    MAX_LENGTH = 4000
    for i in range(0, len(text), MAX_LENGTH):
        ...
        try:
            ...
            req = urllib.request.Request(url, data=data)
            urllib.request.urlopen(req, context=ctx)
            log_message(f"⚠️ 텔레그램 전송 (텍스트 모드): {e}")

def call_gemini_api(prompt):
    if not GEMINI_API_KEY:
        log_message("⚠️ GEMINI_API_KEY가 설정되지 않았습니다.")
        return None
...
def generate_weekend_news():
    ...
    industries = [
        ...
        "차세대 바이오·디지털 헬스케어", "로보틱스와 휴머노이드", "양자 컴퓨팅", "차세대 반도체 및 스마트 팩토리"
    ]
    ...
    prompt = (
        ...
        f"이와 별도로, 이번 주 주말의 **[신산업 심층 포커스]** 섹션을 반드시 포함해야 합니다. 이번 주 선정 산업은 **'{target_new_industry}'**입니다. "
        f"해당 산업의 현재 글로벌 발전 정보와 전문가들의 수치 기반 의견을 바탕으로 심층 분석해 주세요.\n\n"
        f"각 이슈 분석은 1)요약/데이터, 2)관련국 및 지정학적 맥락, 3)긍정/부정 시나리오, 4)한국 시장 기회 및 위협을 포함해야 합니다.\n"
        f"마지막에는 다음 요약을 포함해 주세요:\n"
        f"- [글로벌 마켓 뷰]: 이번 주 글로벌 거시경제 흐름 요약\n"
        f"- [한국의 대응 전략]: 점검 포인트 3가지\n"
    ...
    if content:
        ...
        send_telegram_message(f"⚠️ {today_str_full} 주말 뉴스 생성에 실패했습니다. API 로그를 확인해 주세요.")
